//+------------------------------------------------------------------+
//|                    PROJECT BB BREAKOUT  (MT5 v2.2)                        |
//|  LQS Liquidity Sweep + Bollinger Band Confluence EA             |
//|  Based on LQS Zone Scalper v1.191                               |
//|                                                                  |
//| v2.2: BO_BE_Trigger + BO_BE_Buffer — separate BE for BO entries    |
//|  BO entries use BO_BE_Trigger (default 0.10) — triggers BE much     |
//|  sooner than LQS/BB entries (LQS_BE_Trigger = 0.35). BO position   |
//|  identified by "BO" prefix in position comment. Does not affect     |
//|  LQS/BB trade management at all.                                    |
//|                                                                  |
...
input double BO_BE_Trigger          = 0.10;
// Move SL to breakeven when a BO trade profit reaches this dollar amount.
// Applies ONLY to positions tagged "BO" — does not affect LQS or BB entries.
// Lower than LQS_BE_Trigger (0.35) because BO entries risk sharp reversals
// right after the breakout bar; catching $0.10 peak profit saves the loss.
// Set 0 to disable (BO entries use LQS_BE_Trigger like all other positions).
input double BO_BE_Buffer           = 0.10;
// Dollar buffer beyond entry when BO breakeven triggers.
// BO BUY: SL = entry + BO_BE_Buffer. BO SELL: SL = entry - BO_BE_Buffer.
// Absorbs SL fill slippage so BE never closes at a loss.
...
int OnInit()
{
   ...
   Print("Project BB Breakout v2.2 | Symbol=", Symbol(),
         " | Magic=", MagicNumber,
         " | Exit=", exitMode,
...
void ManageTrades()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      // BO entries: use BO_BE_Trigger (tighter) — detects early reversals
      // LQS/BB entries: use LQS_BE_Trigger (0.35) — unchanged behaviour
      else
      {
         string posComment = PositionGetString(POSITION_COMMENT);
         bool   isBO       = (StringFind(posComment, "BO") == 0);
         // When BO_BE_Trigger=0, BO falls back to LQS_BE_Trigger (matches input comment).
         double beTrigger  = (isBO && BO_BE_Trigger > 0.0) ? BO_BE_Trigger : LQS_BE_Trigger;
         double beBuffer   = (isBO && BO_BE_Trigger > 0.0) ? BO_BE_Buffer  : LQS_BE_Buffer;
         if(beTrigger > 0.0 && profit >= beTrigger)
         {
            double beSL  = (ptype == POSITION_TYPE_BUY)
                           ? NormalizeDouble(op + beBuffer, _Digits)
                           : NormalizeDouble(op - beBuffer, _Digits);
            bool improve = (ptype == POSITION_TYPE_BUY)
                           ? (beSL > newSL + _Point)
...
void TryLQSTrade()
{
   ...
   // BO entries exempt: bar[2] DI spread can pre-date the crossover that triggered
   // the breakout — blocking BO at the exact momentum moment is incorrect.
   if(!isBreakout && LQS_DI_Spread_Filter > 0.0)
   {
   ...
   string info =
      ...
      "  Exit     : " + (LQS_Trail_Start > 0.0
                          ? "Trail +$" + DoubleToString(LQS_Trail_Start, 2)
                            + "  step=$" + DoubleToString(LQS_Trail_Step, 2)
                            + (LQS_BE_Trigger > 0.0
                               ? "  LQS/BB BE=$" + DoubleToString(LQS_BE_Trigger, 2)
                                 + "+buf$" + DoubleToString(LQS_BE_Buffer, 2)
                               : "")
                            + (BO_BE_Trigger > 0.0
                               ? "  BO BE=$" + DoubleToString(BO_BE_Trigger, 2)
