//+------------------------------------------------------------------+
//|              PROJECT BB BREAKOUT  (MT5 v3.1)  — GBPUSD          |
...
//|  Waiting for a retest provides a structurally superior entry    |
//|  with SL anchored to the broken level and natural confirmation  |
//|  that it is holding as support/resistance.                       |
//|  SL recalculated from limit price (same as BB/LQS limit logic). |
//|  Unified BE/trail parameters applied to BOTH BO and BB entries:  |
//|  BE Trigger $0.25, BE Buffer $0.10, Trail Start $0.45,          |
//|  Trail Step $0.30 — consistent risk management across types.    |
//|                                                                  |
...
#property description "Project BB Breakout | LQS + Breakout Continuation | GBPUSD M1 v3.1"
...
// 0.30: wide enough to survive normal oscillation on mean reversion trades.
...
// 0.25: matches BB/LQS BE trigger — consistent risk management across types.
...
// 0.45: matches BB/LQS trail start — unified exit logic across entry types.
// Fixed SL (BO_SL_Dollar) acts as backstop until trail activates.
input double BO_Trail_Step          = 0.30;
// Dollar gap between trailing SL and current BID/ASK for BO entries.
// 0.30: follows momentum $0.30 behind — unified with BB/LQS trail step.
...
int OnInit()
{
   ...
   Print("Project BB Breakout v3.1 (GBPUSD) | Symbol=", Symbol(),
         " | Magic=", MagicNumber,
         " | Exit=", exitMode,
...
void TryLQSTrade()
{
   ...
   if(isBreakout)
   {
      // BO: limit order at the broken swing level — waits for a retest of the
      // breakout level before filling. Provides a structurally superior entry
      // vs chasing momentum at market, with SL anchored to the level.
      // BUY  limit at swing high (broken resistance → now support on retest).
      // SELL limit at swing low  (broken support   → now resistance on retest).
      // Expires in 2 minutes if price does not pull back to the level.
      ...
      // Recalculate SL anchored to limit price.
      double limitSL = sl;
      double limitTP = tp;
      if(slIsFixedDollar)
...
void DrawInfoPanel()
{
   ...
   string info =
      "╔══ PROJECT BB BREAKOUT  v3.1 (GBPUSD)  |  LQS + BB ══╗\n"
      "  Symbol  : " + Symbol()
