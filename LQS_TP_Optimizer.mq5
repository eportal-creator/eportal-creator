//+------------------------------------------------------------------+
//|                  LQS_TP_Optimizer.mq5               v1.00        |
//|                                                                  |
//|  Replays LQS signal detection over a date range using the same   |
//|  filters as ATR_AUTO_LOCK_SCALPER_MT5.mq5, then simulates each   |
//|  trade at 8 TP levels (0.25 → 3.00 × ATR) to find the optimal   |
//|  TP setting.                                                     |
//|                                                                  |
//|  HOW TO USE:                                                     |
//|  1. Attach to XAUUSD M1 chart                                    |
//|  2. Set DateFrom / DateTo to the period you want to analyse      |
//|  3. Match all other inputs to your EA's current settings         |
//|  4. Run — output appears in the Experts / Journal tab            |
//|                                                                  |
//|  SL simulation rule: if both SL and TP are hit on the same M1   |
//|  bar, the close direction is used as the tiebreaker              |
//|  (bearish close → TP likely first for SELL; bullish → SL first). |
//+------------------------------------------------------------------+
#property script_show_inputs
#property copyright "Project ATR"
#property version   "1.00"
#property description "LQS TP Optimizer — compare 8 TP levels across historical LQS signals"
...
input double   M1_Ranging_Thresh     = 25.0;   // M1 ADX below this → ranging SL
...
// Set false for clean summary-only output on large date ranges.

//--- TP levels to test (× ATR)
#define N_TP 8
static const double TP_LEVELS[N_TP] = { 0.25, 0.35, 0.50, 0.75, 1.00, 1.50, 2.00, 3.00 };
static const double TP_CURRENT      = 0.35;   // highlight current EA setting
...
void OnStart()
{
   ...
   for(int i = fromShift; i >= (int)MathMax(toShift, maxFwdBars + 2); i--)
   {
      ...
      {
         double tpDist = atr * TP_LEVELS[tp];
         bool   hit    = false;
         bool   isWin  = false;
   ...
   Print(StringFormat("  %-6s | %4s | %4s | %4s | %6s | %8s | %s",
         "TP×ATR","Wins","Loss","Exp","WinRate","Net P&L","Avg MFE"));
   Print(sep);
   ...
   for(int tp = 0; tp < N_TP; tp++)
   {
      ...
      string mark  = (MathAbs(TP_LEVELS[tp] - TP_CURRENT) < 0.001) ? "  ◄ CURRENT" : "";

      Print(StringFormat("  %-6.2f | %4d | %4d | %4d | %5.1f%% | $%7.2f | %.2f%s",
            TP_LEVELS[tp],
   ...
   Print("  Any TP level above the Avg MFE will rarely be reached → check expiry count.");
   Print("══════════════════════════════════════════════════════════");
}
//+------------------------------------------------------------------+
