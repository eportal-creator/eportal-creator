...
// 0.35: locks in small profit before trail takes over at $0.60.
// Set 0 to disable.
...
// 0.60: gives mean reversion trades room to develop before trailing.
...
// 0.35: wide enough to survive normal oscillation on mean reversion trades.
...
// Recommended: 1.00 for XAUUSD M1 at 0.01 lots.
input double BO_Trail_Start         = 0.20;
// Begin trailing SL for BO entries when profit reaches this dollar amount.
// 0.20: trail kicks in quickly once momentum is confirmed — tighter than
// BB+LQS (0.60) because breakouts stall fast if the move is genuine.
...
input double BO_RSI_Oversold        = 32.0;
// BO SELL blocked when RSI(14) falls below this level.
// Calibrated to block 10:52 BO SELL (RSI=29.85) while allowing
// 09:20 BO SELL (RSI=32.45). Only active when BO_RSI_Filter=true.
input double BO_RSI_Overbought      = 68.0;
// BO BUY blocked when RSI(14) rises above this level.
// Mirror of BO_RSI_Oversold for the BUY side. Only active when BO_RSI_Filter=true.
...
input bool   Enable_Notify       = false;
...
int OnInit()
{
   ...
   Print("Project BB Breakout v2.6 | Symbol=", Symbol(),
         " | Magic=", MagicNumber,
         " | Exit=", exitMode,
...
void ManageTrades()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      // Trail: price-follows SL once profit >= trailStart
      ...
      // Breakeven: BB+LQS / BB only — BO relies on trail for early protection.
...
void DrawInfoPanel()
{
   ...
   string info =
      "╔══ PROJECT BB BREAKOUT  v2.6  |  LQS + Bollinger Bands ══╗\n"
      "  Symbol  : " + Symbol()
