...
//|    M1 DI already confirms direction; applying M5 would block    |
//|    valid pullback BUYs/SELLs during intraday retracements.      |
//|  Mean Reversion / Squeeze: M5 HTF filter still applies          |
//|    (counter-trend entries need HTF confirmation).               |
//|                                                                  |
//| v1.3: Mode-aware direction enforcement                           |
//|  Riding the Bands (ADX >= 25): M1 DI direction is always        |
//|    enforced — uptrend blocks SELLs, downtrend blocks BUYs.      |
//|  No toggle needed; M1 DI has no lag unlike M5 HTF filter.       |
//|  Mean Reversion / Squeeze: unchanged — counter-trend entries    |
//|    still allowed (LQS_M1_DI_Align toggle applies there).        |
//|                                                                  |
...
void TryLQSTrade()
{
   ...
   // ── Entry condition A: LQS swing sweep ───────────────────────
   ...
   bool isSell = lqsSell || bbSell;
   bool isBuy  = lqsBuy  || bbBuy;
   ...
   if(isLQS)
   {
      ...
      // BB-only entries have the touch built into their detection condition,
      // so BB_Require_Band_Touch has no additional effect on that path.
   ...
   // Prevents entries when the signal bar already reversed past the midline —
   // e.g. SELL after bar crashed from upper band all the way below the midline.
   if(BB_Close_Zone_Filter && g_BB_Middle > 0.0)
   {
      if(isSell && c1 < g_BB_Middle) return;
      if(isBuy  && c1 > g_BB_Middle) return;
   }
   ...
   if(m1IsTrending && Riding_Structure_SL && isLQS &&
      g_LQS_SwingHigh > 0.0 && g_LQS_SwingLow > 0.0)
   {
      double structSL = (dir == ORDER_TYPE_BUY)
   ...
   else if(Riding_Structure_SL && !isLQS && g_BB_Upper > 0.0 && g_BB_Lower > 0.0)
   {
   ...
   string entryType = isLQS ? "BB+LQS" : "BB";
   ...
   if(ok)
   {
      ...
      Print("OPEN BB+LQS ", (dir == ORDER_TYPE_BUY ? "BUY " : "SELL"),
